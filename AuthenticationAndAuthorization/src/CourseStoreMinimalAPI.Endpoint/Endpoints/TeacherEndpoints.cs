...
public static class TeacherEndpoints
{
    ...
    public static WebApplication MapTeachers(this WebApplication app, string prefix)
    {
        ...
        teacherGroup.MapPost("/", Insert).DisableAntiforgery().AddEndpointFilter<ValidationFilter<TeacherSaveRequest>>();
        teacherGroup.MapPut("/{id:int}", Update).DisableAntiforgery().AddEndpointFilter<ValidationFilter<TeacherSaveRequest>>();
    ...
    static async Task<Results<Created<TeacherResponse>,ValidationProblem>> Insert([AsParameters]TeacherSaveParameters parameters,
                                                       [FromForm] TeacherSaveRequest request)
    {
        Teacher teacher = parameters.Mapper.Map<Teacher>(request);
        string fileName = DefaultTeacherImageName;
        if (request.File is not null)
        {
            fileName = parameters.FileAdapter.InsertFile(request.File, TeacherImageFolder);
        }
        teacher.ImageUrl = fileName;
        int savedEntityId = await parameters.teacherService.Insert(teacher);
        await parameters.CacheStore.EvictByTagAsync(CacheKey, default);

        var createdTeacherResponse = parameters.Mapper.Map<TeacherResponse>(teacher);
